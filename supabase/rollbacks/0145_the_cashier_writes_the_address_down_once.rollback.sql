-- =============================================================================
-- ROLLBACK 0145 · Devuelve create_business_manual_order al estado de la 0131
-- =============================================================================
--
-- ⚠️ EL ENUM NO SE PUEDE REVERTIR. PostgreSQL no permite quitar un valor de un
--    enum: haría falta recrear el tipo y reescribir toda columna que lo use.
--    `business_created` se queda en `address_source`, y es inofensivo — deja de
--    escribirse y nada más. Las filas que ya lo lleven CONSERVAN ese valor: son
--    direcciones reales creadas por la caja y borrarlas sería perder dato.
--
-- ⚠️ ANTES DE CORRER ESTO. Los pedidos creados bajo la 0145 conservan su
--    `address_directory_id` y sus coordenadas; esto no los toca. Lo que se
--    pierde es que los pedidos NUEVOS vuelvan a nacer sin puntero ni GPS, y con
--    ello la captura del motorizado se queda sin fila destino.
--
--    Revertí también el cliente (`apps/negocios`) y el endpoint: si el API sigue
--    mandando `p_address_directory_id`, PostgREST no encontrará la función y
--    TODA creación de pedido manual fallará. El orden correcto es
--    cliente/endpoint primero, base después.
-- =============================================================================

-- 1. Borrar la firma de 13 argumentos que introdujo la 0145.
...
-- 2. Recrear la de 12 argumentos, cuerpo EXACTO de la 0131.
...
-- Guard: una sola firma, y la de la 0131.
DO $$
DECLARE v_count int;
BEGIN
  ...
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'rollback 0145 incompleto: quedan % firmas', v_count USING errcode = 'P0001';
  END IF;
END $$;
