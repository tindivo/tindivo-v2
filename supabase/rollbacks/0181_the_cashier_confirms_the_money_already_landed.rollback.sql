-- ROLLBACK de 0181 — `validate_order` vuelve a admitir SOLO `validando`.
--
-- Restaura el cuerpo tal cual lo dejó la 0117 (`pg_get_functiondef` verificado
-- contra la definición viva antes de aplicar la 0181).
--
-- OJO: los pedidos que ya se confirmaron por el camino directo NO se tocan.
-- Quedan en `preparing` con `payment_proof_status = verified` y sin
-- `comprobante_prepago_url`, que es exactamente lo que pasó. Para encontrarlos:
--   select o.short_id, o.status, l.created_at
--     from public.order_event_log l
--     join public.orders o on o.id = l.order_id
--    where l.event_type = 'order.payment_confirmed_direct';
