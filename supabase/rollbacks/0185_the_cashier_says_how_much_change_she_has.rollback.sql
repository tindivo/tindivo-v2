-- =============================================================================
-- ROLLBACK 0185 · La cajera dice cuánto vuelto tiene esta noche
--
-- Devuelve R3 a la constante global y quita la columna de la declaración diaria.
--
-- OJO CON EL ORDEN. `create_customer_order` se restaura ANTES de borrar la
-- columna y la función: mientras exista una versión de la RPC que llame a
-- `effective_max_change`, borrarla deja la creación de pedidos rota.
--
-- Este script AVISA si hay declaraciones de vuelto guardadas. No aborta —son
-- datos operativos de noches ya pasadas, no dinero— pero conviene saber cuántas
-- se pierden antes de correrlo.
-- =============================================================================
...
begin
  ...
  from public.business_service_days
  where change_available is not null;

  if v_rows > 0 then
    raise warning 'ROLLBACK 0185: se descartan % declaraciones de vuelto disponible.', v_rows;
  end if;
end $$;

-- ── 1. create_customer_order vuelve a leer app_settings.max_change ───────────
...
-- ── 2. Fuera la función y la columna ─────────────────────────────────────────

drop function if exists public.effective_max_change(uuid);

alter table public.business_service_days
  drop constraint if exists bsd_change_available_chk;

alter table public.business_service_days
  drop column if exists change_available;
