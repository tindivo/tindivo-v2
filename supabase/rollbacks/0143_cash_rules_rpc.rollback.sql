...
--
-- NOTA (corregido). Aquí había un placeholder que abortaba a propósito con
-- "ROLLBACK INCOMPLETO: Insertar aquí el cuerpo de 0105 antes de ejecutar", así
-- que este rollback no se podía ejecutar. Se sustituye por la definición EXACTA
-- de la 0105 (`0105_block_same_business_active_order.sql`, líneas 32-470).
--
-- Al volver a la 0105 se pierden los tres cambios de la 0143:
--   · el guard explícito de `pending_mixed` (el canal B2C vuelve a aceptarlo)
--   · `change_to_give` vuelve a `greatest(0, round(...))`, que oculta un vuelto
--     negativo en vez de dejarlo ver
--   · las validaciones R2 (billete máximo) y R3 (vuelto máximo) desaparecen
