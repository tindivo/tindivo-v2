-- ============================================================================
-- ROLLBACK 0207 — El motorizado sabe de qué calidad es el punto al que va
-- ============================================================================
--
-- Devuelve `create_customer_order` a la firma de 19 argumentos, la que dejó la
-- 0199. Existe por lo mismo que el de aquella: esta es la función que CREA
-- PEDIDOS, y la vuelta atrás no puede depender de que alguien reconstruya 599
-- líneas a mano y con prisa.
--
-- GENERADO POR INVERSIÓN, NO ESCRITO. Sale de deshacer las tres sustituciones
-- de la 0207 sobre su propio texto, y está COMPROBADO: el cuerpo resultante da
-- md5(prosrc) = 92d822551ce7a26ce8b236350d07fa60, que es exactamente el de la
-- versión de 19 argumentos — verificado en local, en prod y en el fichero de
-- la 0199. Se aplicó en local dentro de una transacción y se deshizo, así que
-- la comprobación no dejó rastro.
--
-- LAS COLUMNAS NO SE BORRAN. `delivery_coordinates_accuracy_m` y
-- `delivery_location_confirmed_at` se quedan con lo que ya se hubiera escrito:
-- son datos de pedidos reales y tirarlos no arregla nada. Simplemente dejan de
-- llenarse, y el app del motorizado vuelve a no decir nada de la calidad del
-- punto, que es como estaba antes.
--
-- SI EL API YA ESTÁ DESPLEGADO llamando con los dos parámetros nuevos, hay que
-- volverlo atrás TAMBIÉN: una llamada con `p_delivery_accuracy_m` contra esta
-- firma es un 404 de PostgREST, y no se podría pedir.
-- ============================================================================


-- 1 · Fuera la firma de 21 argumentos
...
-- 2 · La de 19, idéntica a la que dejó la 0199
