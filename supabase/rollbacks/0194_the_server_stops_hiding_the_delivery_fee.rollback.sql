-- Rollback para 0194_the_server_stops_hiding_the_delivery_fee.sql
--
-- Restaura `create_customer_order` tal y como la dejó la 0187, que es la última
-- definición completa anterior. La firma es IDÉNTICA en las dos migraciones
-- (mismos 18 parámetros, mismos defaults), así que basta un `CREATE OR REPLACE`:
-- no hay que borrar ninguna sobrecarga huérfana ni rehacer grants —un replace
-- los conserva—.
--
-- QUÉ VUELVE AL ESTADO ANTERIOR
--   El servidor vuelve a omitir el envío cuando se niega la contraentrega. Ese
--   era el defecto que la 0194 arregló, así que aplicar esto lo reintroduce a
--   conciencia: úsalo solo para desatascar producción, y con el arreglo ya
--   preparado para volver a subir.
--
-- CUIDADO SI HAY MIGRACIONES POSTERIORES
--   Cualquier migración > 0194 que vuelva a tocar `create_customer_order` queda
--   pisada por este rollback, porque esto reescribe la función ENTERA con el
--   cuerpo de la 0187. Antes de correrlo, comprueba que 0194 es de verdad la
--   última que la define:
--     grep -l "FUNCTION public.create_customer_order" supabase/migrations/*.sql
--
