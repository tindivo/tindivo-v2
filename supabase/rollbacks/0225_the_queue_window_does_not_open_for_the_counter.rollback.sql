-- =============================================================================
-- ROLLBACK de la 0225 · devuelve `validate_order` a su cuerpo anterior
-- =============================================================================
--
-- Restaura la definicion EXACTA que la 0225 sustituyo: la que dejo la 0189,
-- verificada byte a byte contra `tindivo-prod` el 2026-09-10
-- (md5 c9d75c3e19b0cb05e6598438c2d82297, 12175 caracteres).
--
-- Aplicarlo devuelve el defecto que la 0225 arregla: `validate_order` vuelve a
-- abrir la ventana de la cola del motorizado en un recojo. No es un incidente
-- por si solo —la policy `ord_driver_read` exige `delivery_method = 'delivery'`
-- y tapa la consecuencia— pero deja al recojo prepagado dependiendo solo de
-- esa policy. Ver la cabecera de la 0225.
-- =============================================================================
