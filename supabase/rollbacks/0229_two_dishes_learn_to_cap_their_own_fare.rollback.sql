-- =============================================================================
-- ROLLBACK de la 0229 · los dos platos dejan de topar su propio envío
-- =============================================================================
--
-- Restaura la definición EXACTA de `create_customer_order` que dejó la 0227
-- (la 0228 no la tocó), copiada literal de
-- `supabase/migrations/0227_one_dish_can_carry_its_own_free_delivery.sql`.
--
...
-- esquema:
--
--   update public.menu_items set flat_delivery_fee = null, flat_delivery_fee_until = null
--    where id in ('0098d55e-2d2a-4013-bb55-dfbd357ee017', '6525d0c3-fa5a-4fcd-bcd5-e0cb16b6f7f4');
--
-- A partir de ese instante los dos vuelven a cobrar la banda completa. Es la
-- vía por defecto ante cualquier susto: no requiere tocar la RPC del checkout.
--
-- EL ORDEN IMPORTA. La función se restaura ANTES de tirar la tabla del ledger
-- y la función pura: la versión de la 0229 inserta en
-- `menu_item_flat_delivery_waivers` y llama a `menu_item_flat_delivery_fee`,
-- así que si esas dos se fueran primero la función viva quedaría rota hasta
-- el `create or replace` de abajo.
--
-- LOS DATOS SE PIERDEN sin vuelta: el DROP TABLE se lleva el ledger completo
-- (cuánto le debe Tindivo al motorizado por esta promo) y el DROP COLUMN los
-- flags de envío fijo. Antes de aplicarlo, si se quiere poder volver:
--
--   select * from public.menu_item_flat_delivery_waivers;
--   select id, name, flat_delivery_fee, flat_delivery_fee_until from public.menu_items
--    where flat_delivery_fee is not null;
-- =============================================================================

-- 1) La función, a su cuerpo anterior (antes de tocar columnas ni funciones).
...
-- 2) El ledger de la promo ya no lo llena nadie.
drop table if exists public.menu_item_flat_delivery_waivers;

-- 3) La función pura del guard nuevo ya no la llama nadie.
drop function if exists public.menu_item_flat_delivery_fee(numeric, date, timestamptz);

-- 4) Las columnas. Esto BORRA los flags de envío fijo por plato configurados.
