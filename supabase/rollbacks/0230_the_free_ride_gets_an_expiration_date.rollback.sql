-- =============================================================================
-- ROLLBACK de la 0230 · el envío gratis por fecha de corte desaparece
-- =============================================================================
--
-- Restaura el estado EXACTO que dejó la 0229 (columnas/función/tabla del
-- envío fijo, y la `create_customer_order`/`cart_item_free_delivery` de esa
-- migración) — porque un rollback deshace SOLO la migración que le da nombre,
-- no juzga si la anterior estaba bien. La 0229 sigue siendo, por decisión de
-- negocio, un mecanismo equivocado (cobraba envío al cliente cuando debía ser
-- gratis, confundiendo comisión con envío). Si lo que se quiere es volver al
-- estado limpio de antes de las dos, aplica ESTE rollback y a continuación
-- `supabase/rollbacks/0229_two_dishes_learn_to_cap_their_own_fare.rollback.sql`.
--
-- EL ROLLBACK RÁPIDO NO ES ESTE ARCHIVO. Es una sentencia, sin deploy y sin
-- migración — apaga el flag de los dos platos sin tocar la función ni el
-- esquema:
--
--   update public.menu_items set free_delivery_until = null
--    where id in ('0098d55e-2d2a-4013-bb55-dfbd357ee017', '6525d0c3-fa5a-4fcd-bcd5-e0cb16b6f7f4');
--
-- EL ORDEN IMPORTA. Las funciones se restauran ANTES de tirar la columna y la
-- función pura de la 0230: la versión viva de `create_customer_order` y
-- `cart_item_free_delivery` leen `free_delivery_until` en un `select` directo,
-- así que si la columna se fuera primero quedarían rotas hasta el
-- `create or replace` de abajo.
--
-- Los datos de `free_delivery_until` SE PIERDEN sin vuelta: el DROP COLUMN se
-- los lleva. Antes de aplicarlo, si se quiere poder volver:
--
--   select id, name, free_delivery_until from public.menu_items
--    where free_delivery_until is not null;
-- =============================================================================

-- 1) Las dos funciones, a su cuerpo anterior (antes de tocar columnas ni
--    funciones nuevas).
...
-- cart_item_free_delivery vuelve a su forma de la 0228 (sin free_delivery_until).
...
-- 2) La función pura del guard de la 0230 ya no la llama nadie.
drop function if exists public.menu_item_free_delivery_until(date, timestamptz);

-- 3) La columna. Esto BORRA los flags de envío gratis por fecha configurados.
alter table public.menu_items
  drop column if exists free_delivery_until;

-- 4) Se restaura la capacidad de la 0229 (flat fee), tal como estaba antes de
--    esta migración.
alter table public.menu_items
