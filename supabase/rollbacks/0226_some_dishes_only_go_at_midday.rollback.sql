-- =============================================================================
-- ROLLBACK de la 0226 · el menu vuelve a no saber de turnos
-- =============================================================================
--
-- Restaura las definiciones EXACTAS que la 0226 sustituyo, capturadas de la base
-- LOCAL el 2026-09-10 antes de aplicarla:
--
--   create_customer_order  (la que dejo la 0220)  32417 chars, md5 ec84e6c190f52e994a6d9482d78b6a23
--   search_catalog         (la que dejo la 0052)  4433 chars, md5 86c7e65ef4bd0b654a2ab4f07c9dd305
--
-- QUE SE PIERDE AL APLICARLO
-- La Florencia vuelve a no poder decir que sus 13 platos de mariscos y de
-- recomendacion del chef son carta de mediodia. Es decir, vuelve al estado en
-- que esos platos llevaban desde el 7-8 de septiembre de 2026: apagados a mano
-- las 24 horas de los 7 dias, porque encenderlos el sabado a las 11:00 y
-- apagarlos a las 15:00 son 26 toques de switch por fin de semana.
--
-- EL ORDEN IMPORTA. Las funciones se restauran ANTES de tirar las columnas:
-- `search_catalog` es `language sql` y su cuerpo se valida al crearla, asi que
-- si las columnas ya no existen el CREATE OR REPLACE falla. Y la version nueva
-- de `create_customer_order` referencia `v_menu_item.available_days`, asi que
-- tirar las columnas con ella viva dejaria la RPC del checkout rota.
--
-- Los datos de las franjas SE PIERDEN sin vuelta: el DROP COLUMN se los lleva.
-- Antes de aplicarlo, si se quiere poder volver:
--
--   select id, name, available_days, available_from, available_to
...
-- 1) Las funciones, a su cuerpo anterior (antes de tocar las columnas).
...
-- 2) Las funciones auxiliares de la franja ya no las llama nadie.
drop function if exists public.menu_item_orderable_now(smallint[], time, time);
drop function if exists public.menu_item_in_window(smallint[], time, time, timestamptz, int);

-- 3) Las columnas. Esto BORRA las franjas configuradas.
alter table public.menu_items
  drop column if exists available_days,
  drop column if exists available_from,
  drop column if exists available_to;

-- 4) El margen de gracia deja de tener sentido sin el guard.
update public.app_settings
   set value = value - 'shiftAvailabilityGraceMinutes'
 where key = 'timers';
