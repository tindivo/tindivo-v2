-- =============================================================================
-- ROLLBACK de 0187 · La primera entrega del lanzamiento la pagamos nosotros
-- =============================================================================
--
-- ESTE ARCHIVO NO ES EL FRENO RÁPIDO. Antes de ejecutarlo, considerar:
--
--   -- Apagar la promo. Un UPDATE, sin deploy y sin migración.
--   update public.app_settings
--      set value = jsonb_set(value, '{active}', 'false'::jsonb)
--    where key = 'promo_free_delivery';
--
--   -- O congelarla sin apagarla: el checkout pasa a decir "agotada", que es
--   -- cierto, y las reservas en vuelo liquidan normal.
--   update public.app_settings
--      set value = jsonb_set(value, '{max_redemptions}', '0'::jsonb)
--    where key = 'promo_free_delivery';
--
-- Cualquiera de las dos deja de aplicar la promo a pedidos NUEVOS al instante.
-- Los pedidos ya creados con S/0 se respetan: su precio ya está en la fila. Esa
-- es la vía por defecto ante un susto un jueves a las 21:00. Este archivo es
-- para deshacer el ESQUEMA, que es otra cosa y casi nunca es lo que hace falta.
--
-- DOS COSAS QUE ESTE ROLLBACK NO HACE, A PROPÓSITO
--
--   1. NO borra `promo_redemptions`. El requisito pedía que la auditoría
--      sobreviviera al fin de la promo; con más razón sobrevive a un rollback.
--      Si algún día hay que purgarla, que sea un script aparte, a mano y con
--      alguien mirando.
--
--   2. NO restaura el CHECK de `orders.delivery_fee_source` a ('business',
--      'system'). Si ya existe un pedido con 'promo', ese ALTER FALLA y deja el
--      rollback a medias — en el peor momento posible. El CHECK ancho se queda:
--      un valor permitido que nadie escribe no hace daño, y borrar el marcador
--      de los pedidos que sí usaron la promo sería destruir la evidencia justo
--      para poder revertir.
--
-- EL ORDEN IMPORTA. La configuración se borra al FINAL (paso 5), después de
-- restaurar la función (paso 1). Al revés queda una ventana en la que la función
-- vieja-con-promo hace `select ... for update` sobre una fila que ya no existe.
-- No regalaría nada —el `if found` lo cubre— pero es una ventana innecesaria.
-- =============================================================================


-- ── 1 · create_customer_order vuelve al cuerpo de 0185 ───────────────────────
--
-- Cuerpo literal de 0185, extraído del propio archivo de migración y verificado
-- byte a byte contra el `pg_get_functiondef` que estaba vivo en prod antes de
-- 0187 (md5 323b4269fe92be7c762b4d9d085e434d). No está escrito a mano.
--
-- Es la parte que hay que tener PREPARADA antes del push, no improvisada
-- durante la caída.
...
-- ── 2 · El liquidador de reservas ────────────────────────────────────────────

drop trigger if exists trg_promo_settle_redemption on public.orders;
drop function if exists public.promo_settle_redemption();


-- ── 3 · El contador ──────────────────────────────────────────────────────────

drop function if exists public.admin_promo_free_delivery_stats();


-- ── 4 · La elegibilidad ──────────────────────────────────────────────────────

drop function if exists public.current_customer_promo_free_delivery();


-- ── 5 · La configuración (AL FINAL, ver cabecera) ────────────────────────────

delete from public.app_settings where key = 'promo_free_delivery';


-- ── Comprobación posterior ───────────────────────────────────────────────────
--
--   -- Debe devolver UNA fila, sin la promo dentro:
--   select md5(pg_get_functiondef(oid)) from pg_proc where proname = 'create_customer_order';
--   -- esperado: 323b4269fe92be7c762b4d9d085e434d
--
--   -- Debe seguir existiendo, con sus filas intactas:
--   select status, count(*) from public.promo_redemptions group by status;
