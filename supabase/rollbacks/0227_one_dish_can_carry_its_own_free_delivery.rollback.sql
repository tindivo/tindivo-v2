-- =============================================================================
-- ROLLBACK de la 0227 · el plato deja de poder traer su propio envío gratis
-- =============================================================================
--
-- Restaura la definición EXACTA de `create_customer_order` que dejó la 0226,
-- copiada literal de `supabase/rollbacks/0226_some_dishes_only_go_at_midday.rollback.sql`
-- (que a su vez la capturó de la base local el 2026-09-10).
--
-- EL ROLLBACK RÁPIDO NO ES ESTE ARCHIVO. Es una sentencia, sin deploy y sin
-- migración — apaga el flag del plato sin tocar la función ni el esquema:
--
--   update public.menu_items set free_delivery_days = null
--    where id = 'f8877a34-6b3e-4928-a9e6-9c87499b110b';
--
-- A partir de ese instante ese plato vuelve a cobrar envío. Es la vía por
-- defecto ante cualquier susto: no requiere tocar la RPC del checkout.
--
-- EL ORDEN IMPORTA. La función se restaura ANTES de tirar la columna y la
-- función pura: la versión de la 0227 lee `mi.free_delivery_days` en un
-- `select` directo (no vía `v_menu_item`), así que si la columna se fuera
-- primero la función viva quedaría rota hasta el `create or replace` de abajo.
--
-- Los datos de `free_delivery_days` SE PIERDEN sin vuelta: el DROP COLUMN se
-- los lleva. Antes de aplicarlo, si se quiere poder volver:
--
--   select id, name, free_delivery_days from public.menu_items
...
-- 2) La función pura del guard nuevo ya no la llama nadie.
drop function if exists public.menu_item_free_delivery_day(smallint[], timestamptz);

-- 3) La columna. Esto BORRA los flags de envío gratis por plato configurados.
alter table public.menu_items
  drop column if exists free_delivery_days;
