-- ============================================================================
-- ROLLBACK 0199 — La nota al motorizado, por fin con un camino de escritura
-- ============================================================================
--
-- Devuelve `create_customer_order` a la firma de 18 argumentos, la que hoy
-- corre en produccion. Existe porque esta es la funcion que CREA PEDIDOS: si
-- algo sale mal despues del push, la vuelta atras no puede depender de que
-- alguien reconstruya 597 lineas a mano y con prisa.
--
-- COMO SE GENERO, Y POR QUE SE PUEDE CONFIAR EN EL.
--
--   No esta escrito a mano: sale de invertir las tres sustituciones de la 0199
--   sobre su propio texto —quitar el parametro de la firma, quitar las dos
--   lineas de `customer_notes` del cuerpo y devolver la coma que cada una le
--   habia robado a la linea anterior—.
--
--   Y esta COMPROBADO contra la definicion viva en `tindivo-prod`, no supuesto:
--
--     md5(pg_get_functiondef(...)) en prod = 136cdb4ceb8fa38fba109fb84ccf432e
--     md5(prosrc)                  en prod = 8ca8c88fd551b3fc2689c64b55ba2438
--
--   El cuerpo que recrea este fichero da ese mismo md5 de `prosrc`, byte a
--   byte. Se verifico aplicandolo en local dentro de una transaccion y
--   deshaciendola, asi que la comprobacion no dejo rastro.
--
-- LO QUE SE PIERDE AL VOLVER: la columna `orders.customer_notes` sigue
-- existiendo y sigue teniendo lo que ya se escribio —esto no borra ninguna
-- nota—, pero deja de llenarse. El app del motorizado la pinta cuando la hay,
-- asi que simplemente vuelve a no haberla. Si el API ya esta desplegado con
-- `p_customer_notes`, hay que volverlo atras TAMBIEN: una llamada con ese
-- argumento contra esta firma es un 404 de PostgREST, y no se podria pedir.
-- ============================================================================


-- 1 · Fuera la firma de 19 argumentos
...
-- 2 · La de 18, identica a la que corria antes de la 0199
...
-- 3 · Los grants, como los deja la 0199 (y como estaban antes)
...
GRANT EXECUTE ON FUNCTION public.create_customer_order(
  uuid, uuid, public.delivery_method, public.payment_intent, text, text, jsonb,
  text, text, numeric, numeric, public.order_source, numeric,
  double precision, double precision, double precision, numeric, text) TO service_role;
