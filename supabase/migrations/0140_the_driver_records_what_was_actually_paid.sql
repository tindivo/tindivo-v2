-- =============================================================================
-- 0140 · El motorizado registra el cobro REAL, no el planeado.
--
-- EL PROBLEMA.
--   El cliente cambia de idea en la puerta: dijo efectivo y paga por Yape, o
--   parte y parte. Hasta ahora `deliver` solo guardaba `payment_real`, asi que
--   de las tres cosas que definen el cobro —metodo, division y billete— se
--   registraba una. Consecuencia concreta: un pedido `pending_mixed` pagado tal
--   cual el plan NO se podia contar con verdad, porque la hoja obligaba a
--   elegir todo-efectivo o todo-Yape, y las dos mentian. Con ~1% de pedidos
--   mixtos eso es un descuadre cada pocos dias, y siempre lo paga alguien: o el
--   motorizado rinde de mas, o el negocio cobra de menos.
--
-- QUE HACE ESTA MIGRACION.
--   `advance_order('deliver')` pasa a aceptar `cashAmount`, `yapeAmount` y
--   `clientPaysWith`, los VALIDA en el servidor y deriva `cash_owed_at_delivery`.
--
-- POR QUE `cash_owed_at_delivery` Y NO UN `case` EN CADA CONSUMIDOR.
--   La columna existe desde 0002 y no la escribia ni la leia nadie. Es
--   exactamente la pregunta que el corte de caja necesita —cuanto efectivo se
--   lleva el motorizado por este pedido— y hoy esa respuesta se DEDUCE del
--   metodo en cuatro sitios distintos (0018, 0111 x2, y el endpoint de
--   efectivo del motorizado). Cuatro copias de una regla divergen; esta app ya
--   lo ha demostrado varias veces. Escribiendola una vez en la entrega, la
--   liquidacion deja de interpretar y solo suma — y el dia que aparezca otra
--   forma de pago, no se entera.
--
--   Los consumidores pasan a leerla en 0141, junto con el backfill de las filas
--   ya entregadas. Esta migracion sola no cambia ningun importe: solo empieza a
--   rellenar la columna.
--
-- LO QUE NO TOCA.
--   Ni comisiones ni cargos al negocio: `generate_delivery_charges` (0074) es un
--   trigger que solo lee `delivery_fee_charged`, `delivery_fee`,
--   `commission_amount` y `tindivo_commission`. Lo que el restaurante le debe a
--   Tindivo es identico pague el cliente como pague.
--
-- METODO.
--   El cuerpo se genero desde `pg_get_functiondef` de la definicion VIVA (la de
--   0128), tocando UNICAMENTE la rama `deliver`. El resto es identico byte a
--   byte: esta funcion tiene ~390 lineas y transcribirla a mano es como se
--   pierden ramas.
--
