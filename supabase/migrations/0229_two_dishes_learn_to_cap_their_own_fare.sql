-- =============================================================================
-- 0229 · Dos platos aprenden a topar su propio envío
--
-- Idempotente (add column if not exists / create or replace / create table if
-- not exists). Rollback en
-- supabase/rollbacks/0229_two_dishes_learn_to_cap_their_own_fare.rollback.sql
-- =============================================================================
--
-- EL CASO
-- Al Punto lanza "Promoción 6 Alitas Crispy" y "Promoción Combo Familiar
-- Deluxe" con envío TOPADO a S/1.50 (no gratis) hasta el 27 de septiembre de
-- 2026 inclusive. Distinto de la 0227 en dos ejes a propósito:
--   · NO es gratis. Es un monto fijo menor que la banda normal (S/2.00 cerca,
--     S/2.50 lejos), así que sigue habiendo algo que cobrar y algo que
--     registrar como diferencia.
--   · Tiene FECHA DE CORTE, no un calendario semanal. "Hasta el 27" es una
--     ventana con fin, no un patrón de días que se repite indefinidamente.
--
-- QUIÉN PAGA LA DIFERENCIA (decidido con el negocio antes de escribir esto,
-- mismo criterio que la 0227): Tindivo, no el negocio ni el motorizado. Por
-- eso esta migración SÍ necesita un ledger —`menu_item_flat_delivery_waivers`—
-- que la 0227 no necesitó: ahí el envío se iba a cero y no había nada que
-- reconciliar; acá el motorizado sigue cobrando la banda completa al cliente,
-- Tindivo le debe la diferencia, y hace falta un rastro de cuánto es.
--
-- MISMO CRITERIO DE CARRITO Y CANAL QUE LA 0227 (decidido con el negocio):
--   · Solo si el carrito es ÚNICAMENTE ese plato. Si lleva algo más, se cobra
--     el envío completo — un pedido de S/100 no se cuelga de un combo de
--     S/17.90 para pagar S/1.50 de envío.
--   · Solo canal `customer_pwa`. Lo financia Tindivo, no es un default que
--     deba activarse solo en un pedido que teclea la cajera.
--   · `pickup` queda fuera por construcción: ahí el envío ya es 0 (0185).
--
-- POR QUÉ UNA FUNCIÓN PURA APARTE (`menu_item_flat_delivery_fee`)
-- Mismo argumento que `menu_item_free_delivery_day` (0227): separada, `stable`,
-- sin leer tablas, se puede probar con valores de mano sin depender de la
-- fecha real. FAIL-CLOSED en todo: `flat_delivery_fee` o
-- `flat_delivery_fee_until` en NULL nunca aplican — un dato roto acá no cuesta
-- una venta, cuesta plata de Tindivo regalada sin fecha de corte.
--
-- LA FRONTERA DEL DÍA ES LA JORNADA OPERATIVA, NO EL RELOJ. Misma fórmula que
-- la 0227 (arranca a las 05:00 de Lima), reimplementada a propósito en vez de
-- llamar a `current_service_date()`, por la misma razón: no depender de sus
-- GRANTs desde una función nueva, y poder pasarle un `p_at` de prueba.
--
-- QUÉ NO HACE ESTA MIGRACIÓN: no toca el checkout del cliente (equivalente a
-- la 0228 para esta promo). La pantalla de pago seguirá mostrando la banda
-- nominal (S/2.00/S/2.50) tachada hasta que el pedido se confirme, donde el
-- servidor SÍ cobra S/1.50 de verdad. Es la misma brecha que la 0228 cerró
-- para la 0227 — se deja fuera de esta migración a propósito porque implica
-- cambios de frontend (hook + tipos), no solo SQL.
-- =============================================================================

alter table public.menu_items
  add column if not exists flat_delivery_fee numeric(10,2)
    check (flat_delivery_fee is null or flat_delivery_fee >= 0),
  add column if not exists flat_delivery_fee_until date;

comment on column public.menu_items.flat_delivery_fee is
  'Envío fijo que se cobra en vez de la banda normal, financiado por Tindivo (la diferencia se registra en menu_item_flat_delivery_waivers). NULL = sin promo (fail-closed). Solo aplica si el pedido es ÚNICAMENTE este plato, por canal customer_pwa, en modo delivery, y solo hasta flat_delivery_fee_until.';
comment on column public.menu_items.flat_delivery_fee_until is
  'Último día (jornada operativa de Lima, 05:00) en que flat_delivery_fee aplica, inclusive. NULL = nunca (fail-closed).';

-- -----------------------------------------------------------------------------
-- ¿Cuál es el envío fijo de este plato en el instante `p_at`, si aplica?
--
-- Pura: no lee ninguna tabla. FAIL-CLOSED: fee o fecha en NULL, fee negativo, o
-- fecha ya pasada devuelven NULL (no aplica). Devuelve el monto a cobrar
-- (nunca un booleano) porque a diferencia de la 0227 el resultado no es
-- "gratis o no": es "¿cuánto, si algo distinto de lo normal?".
-- -----------------------------------------------------------------------------
create or replace function public.menu_item_flat_delivery_fee(
  p_flat_delivery_fee numeric,
  p_flat_delivery_fee_until date,
...
as $function$
  select case
    when p_flat_delivery_fee is not null
     and p_flat_delivery_fee >= 0
     and p_flat_delivery_fee_until is not null
     and (
       -- Mismo cálculo que la 0227: la jornada de Lima empieza a las 05:00,
       -- así que la madrugada cuenta como el día anterior.
       (timezone('America/Lima', coalesce(p_at, now())) - interval '5 hours')::date
     ) <= p_flat_delivery_fee_until
    then p_flat_delivery_fee
    else null
  end
$function$;

comment on function public.menu_item_flat_delivery_fee(numeric, date, timestamptz) is
  'Fail-closed: fee o fecha de corte en NULL, fee negativo, o fecha ya pasada devuelven NULL. Jornada operativa (05:00 Lima), no el reloj.';

-- Superficie cerrada (patrón 0009/0204/0227): solo la llama
-- create_customer_order, que es SECURITY DEFINER de postgres.
revoke all on function public.menu_item_flat_delivery_fee(numeric, date, timestamptz)
  from public, anon, authenticated;
grant execute on function public.menu_item_flat_delivery_fee(numeric, date, timestamptz)
  to service_role;

-- -----------------------------------------------------------------------------
-- Ledger de lo que Tindivo le debe a quien cobra el envío (el motorizado sigue
-- cobrando la banda completa al cliente; esta fila es la diferencia a
-- reconciliar). Una fila por pedido, nunca se actualiza: es auditoría, no
-- estado. A diferencia de `promo_redemptions` (0187) no hay tope global ni
-- límite por cuenta que proteger, así que no hace falta el lifecycle
-- reserved/redeemed/released: se inserta una sola vez, al momento de decidir
-- el precio, dentro de la misma transacción que el pedido.
-- -----------------------------------------------------------------------------
create table if not exists public.menu_item_flat_delivery_waivers (
  id                    uuid primary key default gen_random_uuid(),
  order_id              uuid not null unique references public.orders(id) on delete cascade,
  menu_item_id          uuid not null references public.menu_items(id) on delete restrict,
  business_id           uuid not null references public.businesses(id) on delete restrict,
  distance_band         public.distance_band,
  normal_delivery_fee   numeric(10,2) not null check (normal_delivery_fee >= 0),
  charged_delivery_fee  numeric(10,2) not null check (charged_delivery_fee >= 0),
  waived_amount         numeric(10,2) not null check (waived_amount >= 0),
  created_at            timestamptz not null default now()
);

comment on table public.menu_item_flat_delivery_waivers is
  'Auditoría de la promo de envío fijo por plato (0229). La suma de waived_amount sobre pedidos delivered es lo que Tindivo debe reconciliar con el motorizado.';
comment on column public.menu_item_flat_delivery_waivers.waived_amount is
  'normal_delivery_fee - charged_delivery_fee. Lo que Tindivo cubre, no el negocio ni el motorizado.';

create index if not exists menu_item_flat_delivery_waivers_business_idx
  on public.menu_item_flat_delivery_waivers (business_id);
create index if not exists menu_item_flat_delivery_waivers_item_idx
  on public.menu_item_flat_delivery_waivers (menu_item_id);

-- RLS (invariante 3). Nadie escribe desde el navegador: la única escritura
-- viene de create_customer_order, SECURITY DEFINER.
alter table public.menu_item_flat_delivery_waivers enable row level security;

drop policy if exists midfw_admin_read on public.menu_item_flat_delivery_waivers;
create policy midfw_admin_read on public.menu_item_flat_delivery_waivers for select to authenticated
  using ((select public.current_user_has_role('admin')));

grant select on public.menu_item_flat_delivery_waivers to authenticated;
grant select, insert on public.menu_item_flat_delivery_waivers to service_role;

-- -----------------------------------------------------------------------------
-- `create_customer_order` con el guard de envío fijo por plato. El resto del
-- cuerpo es la definición que dejó la 0227, copiada tal cual: este bloque solo
-- AÑADE cuatro variables y el `if` nuevo, colocado justo detrás del bloque de
-- envío gratis por plato (0227) y antes de las validaciones de pago — esas
-- validaciones leen `v_delivery_fee`, que es lo que este bloque puede cambiar.
-- Si la 0187 o la 0227 ya dejaron el envío en 0, este bloque no hace nada: el
-- guard de `v_delivery_fee > 0` lo salta solo, porque gratis siempre gana.
-- -----------------------------------------------------------------------------
...
declare
  ...
  -- Promo de envío fijo por plato (0229)
  v_flat_delivery_fee numeric;
  v_flat_delivery_fee_until date;
  v_flat_delivery_fee_applied numeric;
  v_flat_delivery_waived numeric;
  v_order_id uuid;
  v_short_id text;
  v_order_number int;
...
begin
  ...
  -- ── PROMO DE ENVÍO FIJO POR PLATO (0229) ───────────────────────────────────
  -- Mismo criterio que la 0227 ("únicamente ese plato", solo customer_pwa,
  -- solo si aún queda algo que cobrar), pero en vez de regalar el envío lo BAJA
  -- a un monto fijo hasta una fecha de corte. `v_delivery_fee > 0` hace que si
  -- la 0187 o la 0227 ya dejaron el envío en 0, este bloque no tenga nada que
  -- hacer: gratis siempre gana sobre un monto fijo mayor que cero.
  if p_source = 'customer_pwa'
     and p_delivery_method = 'delivery'
     and v_delivery_fee > 0
  then
    ...
    if v_item_promo_distinct = 1 then
      select mi.flat_delivery_fee, mi.flat_delivery_fee_until
        into v_flat_delivery_fee, v_flat_delivery_fee_until
        from public.menu_items mi
       where mi.id = (p_items -> 0 ->> 'menu_item_id')::uuid
         and mi.business_id = p_business_id;

      v_flat_delivery_fee_applied := public.menu_item_flat_delivery_fee(
        v_flat_delivery_fee, v_flat_delivery_fee_until
      );

      -- Solo si el fijo es de verdad MENOR que lo que ya se iba a cobrar: un
      -- fijo mayor o igual a la banda no es una promo, y no debe registrarse
      -- como diferencia regalada.
      if v_flat_delivery_fee_applied is not null
         and v_flat_delivery_fee_applied < v_delivery_fee
      then
        v_flat_delivery_waived := v_delivery_fee - v_flat_delivery_fee_applied;

        insert into public.menu_item_flat_delivery_waivers (
          order_id, menu_item_id, business_id, distance_band,
          normal_delivery_fee, charged_delivery_fee, waived_amount
        ) values (
          v_order_id, (p_items -> 0 ->> 'menu_item_id')::uuid, p_business_id, v_band,
          v_delivery_fee, v_flat_delivery_fee_applied, v_flat_delivery_waived
        );

        v_delivery_fee := v_flat_delivery_fee_applied;
        v_fee_source   := 'promo';
      end if;
    end if;
  end if;
...
-- -----------------------------------------------------------------------------
-- El flag real: los dos combos de Al Punto cobran S/1.50 de envío (en vez de
-- la banda normal) si son lo único en el carrito, hasta el 27 de septiembre
-- de 2026 inclusive. Específico de este lanzamiento — la columna y la función
-- de arriba son la capacidad genérica; esto es el único dato que depende del
-- caso real. No hace nada en una base que no tenga estos platos (0 filas, sin
-- error).
-- -----------------------------------------------------------------------------
update public.menu_items
   set flat_delivery_fee = 1.50,
       flat_delivery_fee_until = '2026-09-27'
 where business_id = '14f7a752-dac0-4332-9930-b09079521af1'
   and id in (
     '0098d55e-2d2a-4013-bb55-dfbd357ee017', -- Promoción 6 Alitas Crispy
     '6525d0c3-fa5a-4fcd-bcd5-e0cb16b6f7f4'  -- Promoción Combo Familiar Deluxe
   );
