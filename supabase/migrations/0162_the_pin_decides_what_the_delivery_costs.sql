-- =============================================================================
-- 0162 · El envio del cliente deja de cobrar «cerca» pase lo que pase
-- =============================================================================
--
...
--
-- `'near'` LITERAL. Daba igual donde cayera el pin: el pedido del cliente
-- cobraba S/ 2.00 siempre, y `delivery_distance_band` ni se escribia -- no
-- estaba en la lista de columnas del INSERT. Medido antes de esta migracion:
-- 13 pedidos `customer_pwa`, los 13 con la banda en NULL.
--
-- Es EXACTAMENTE el mismo defecto que la 0126 arreglo para el pedido manual
-- ("daba igual a donde fuera el pedido: S/ 2.00"), que en el lado del cliente
-- nadie volvio a tocar. Aquella migracion aviso de la trampa: cambiar la firma
-- sin cambiar el calculo deja botones decorativos. Aqui se cambia el calculo.
--
-- QUE HACE AHORA
-- La banda sale de la coordenada, via `delivery_band_for_point` (0161): 'far' si
-- el pin cae en alguna zona lejana activa, 'near' si no. Mientras nadie dibuje
-- una zona esa funcion devuelve 'near' para todo punto, asi que el dia que se
-- aplique esta migracion NINGUN precio cambia. El precio cambia cuando el admin
-- dibuja, que es donde tiene que estar la decision.
--
-- `delivery_fee_source = 'system'` porque lo decidio la geometria. En el pedido
-- manual lo pone la cajera y ahi vale 'business' -- asi los reportes pueden
-- distinguir un envio que eligio una persona de uno que salio del mapa.
--
-- PICKUP: banda NULL y envio 0, misma decision que 0126. El recojo no tiene
-- banda; escribir 'near' seria meter un dato falso en los reportes.
--
-- LA CAJERA NO SE TOCA. `create_business_manual_order` sigue igual: ella elige
-- con sus dos botones y el GPS no entra ahi. Ya sabe si es cerca o lejos.
--
-- LO QUE **NO** SE ARREGLA AQUI, y conviene saberlo: los 13 pedidos que ya
-- existen se quedan con la banda en NULL. No se rellenan porque no hay dato con
-- el que hacerlo -- no se sabe que zonas habia cuando se crearon, y adivinarlas
-- con las de hoy seria inventar historia contable de pedidos ya cobrados.
--
-- El cuerpo sale de `pg_get_functiondef` de la definicion viva (0148), cambiando
-- UNICAMENTE el calculo de la banda y las dos columnas que ahora se escriben.
-- Idempotente.
-- =============================================================================
