-- =============================================================================
-- 0230 · El envío gratis puede tener fecha de vencimiento
--
-- Idempotente (create or replace / add column if not exists / drop if
-- exists). Rollback en
-- supabase/rollbacks/0230_the_free_ride_gets_an_expiration_date.rollback.sql
-- =============================================================================
--
-- LA 0229 ENTENDIÓ MAL EL CASO. Cobraba S/1.50 de envío AL CLIENTE en los dos
-- combos de Al Punto, con un ledger para que Tindivo "cubriera la diferencia"
-- con el motorizado. Correción del negocio: el S/1.50 no tiene nada que ver
-- con lo que paga el cliente — es la COMISIÓN que Tindivo ya le cobra al
-- negocio por cualquier pedido de delivery (`app_settings.commissions.delivery
-- = 1.50`, sin override para Al Punto: `commission_override_delivery IS
-- NULL`). Esa comisión no cambia con esta promo, PORQUE NUNCA CAMBIÓ: se
-- computa en `advance_order` al marcar `pickup` (0224), un camino que esta
-- migración no toca.
--
-- Lo que el negocio pidió de verdad es MÁS SIMPLE que lo que se construyó: los
-- dos combos llevan envío GRATIS para el cliente, hasta el 27 de septiembre de
-- 2026 inclusive. Es exactamente el mismo mecanismo que la 0227 (envío gratis
-- por plato, "únicamente ese plato", solo `customer_pwa`) con un solo cambio de
-- forma: en vez de un calendario semanal (`free_delivery_days`, martes y
-- jueves para siempre) es una VENTANA CON FECHA DE CORTE.
--
-- POR QUÉ ESTA MIGRACIÓN REVIERTE LA 0229 EN VEZ DE DEJARLA AL LADO: cero
-- pedidos y cero filas en `menu_item_flat_delivery_waivers` la usaron (se
-- verificó contra prod antes de escribir esto), así que no hay dato real que
-- conservar. Dejar las columnas `flat_delivery_fee`/`flat_delivery_fee_until`,
-- la función y el ledger sin ningún llamador sería la abstracción sin
-- implementación que este repo evita a propósito — mejor quitar la pieza
-- equivocada que acumular dos caminos para el mismo problema.
--
-- POR QUÉ UNA COLUMNA NUEVA (`free_delivery_until`) Y NO REUTILIZAR
-- `free_delivery_days`: son preguntas distintas. "Días" es un patrón que se
-- repite cada semana sin fin (la promo de alitas de la 0227 sigue viva los
-- martes y jueves). "Hasta una fecha" es una ventana que se cierra sola y no
-- vuelve. Forzar la fecha dentro del array de días habría exigido codificar un
-- calendario entero de casillas true/false, mucho más frágil que un solo
-- `date`.
--
-- LAS DOS CONDICIONES SE COMBINAN CON "O", NO SE EXCLUYEN: un plato puede algún
-- día tener las dos (gratis los martes Y ADEMÁS gratis por lanzamiento hasta
-- tal fecha) sin que una le quite el paso a la otra. Ninguno de los dos platos
-- de este caso usa `free_delivery_days`, así que en la práctica de hoy solo
-- corre la fecha.
--
-- MISMO CANAL Y MISMO CRITERIO DE CARRITO QUE LA 0227 (sin cambios, decidido
-- con el negocio esa vez): solo `customer_pwa`, solo si el pedido es
-- ÚNICAMENTE ese plato.
--
-- EL CHECKOUT TAMBIÉN SE CORRIGE AQUÍ. La 0229 dejó pendiente a propósito el
-- equivalente de la 0228 (pintar el envío gratis ANTES de confirmar) porque
-- implicaba tocar el frontend. Esta migración no necesita tocarlo: extiende
-- `cart_item_free_delivery` (0228) para que también mire
-- `free_delivery_until`, y el hook de checkout que YA la llama
-- (`use-checkout-hooks/use-checkout-state.ts`) empieza a pintar S/0 sin ningún
-- cambio de TypeScript — la pantalla de pago solo conocía un booleano, y este
-- sigue siendo un booleano.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1) DESHACER LA 0229 · nadie la usó, no hay dato que conservar.
-- -----------------------------------------------------------------------------
drop table if exists public.menu_item_flat_delivery_waivers;

drop function if exists public.menu_item_flat_delivery_fee(numeric, date, timestamptz);

alter table public.menu_items
  drop column if exists flat_delivery_fee,
  drop column if exists flat_delivery_fee_until;

-- -----------------------------------------------------------------------------
-- 2) LA CAPACIDAD CORRECTA · envío gratis por plato, con fecha de corte.
-- -----------------------------------------------------------------------------
alter table public.menu_items
  add column if not exists free_delivery_until date;

comment on column public.menu_items.free_delivery_until is
  'Último día (jornada operativa de Lima, 05:00) en que este plato trae envío gratis, financiado por Tindivo. NULL = nunca (fail-closed). Se combina con free_delivery_days por "o": cualquiera de las dos que aplique basta. Solo aplica si el pedido es ÚNICAMENTE este plato, por canal customer_pwa, en modo delivery.';

-- ¿Sigue vigente la ventana de envío gratis de este plato en el instante
-- `p_at`? Pura, fail-closed (NULL o fecha ya pasada → false). Misma fórmula de
-- jornada operativa que `menu_item_free_delivery_day` (0227): reimplementada a
-- propósito, no llamada, por la misma razón de esa migración.
create or replace function public.menu_item_free_delivery_until(
  p_free_delivery_until date,
...
as $function$
  select coalesce(
    p_free_delivery_until is not null
    and (timezone('America/Lima', coalesce(p_at, now())) - interval '5 hours')::date
          <= p_free_delivery_until,
    false
  )
$function$;

comment on function public.menu_item_free_delivery_until(date, timestamptz) is
  'Fail-closed: NULL o fecha ya pasada nunca aplican. Jornada operativa (05:00 Lima), no el reloj. Hermana de menu_item_free_delivery_day (0227): esta mira una fecha de corte, esa un calendario semanal.';

-- Superficie cerrada (patrón 0009/0204/0227): solo la llaman
-- create_customer_order (SECURITY DEFINER de postgres) y cart_item_free_delivery
-- (SECURITY DEFINER, que corre con los privilegios de su dueño).
revoke all on function public.menu_item_free_delivery_until(date, timestamptz)
  from public, anon, authenticated;
grant execute on function public.menu_item_free_delivery_until(date, timestamptz)
  to service_role;

-- -----------------------------------------------------------------------------
-- `create_customer_order` con el guard de fecha añadido al bloque de envío
-- gratis por plato de la 0227. El resto del cuerpo es la definición que dejó
-- la 0227 (0229 solo le había añadido el bloque de envío fijo que aquí se
-- retira): añade una variable y cambia el `if` del bloque de envío gratis,
-- nada más.
-- -----------------------------------------------------------------------------
...
declare
  ...
  -- Promo de envío gratis por plato (0227: calendario semanal; 0230: fecha de corte)
  v_item_promo_distinct int;
  v_item_promo_days smallint[];
  v_item_free_delivery_until date;
  v_order_id uuid;
  v_short_id text;
  v_order_number int;
...
begin
  ...
  -- ── PROMO DE ENVÍO GRATIS POR PLATO (0227: días de semana; 0230: fecha de
  -- corte) ─────────────────────────────────────────────────────────────────
  -- Mismo sitio y mismo criterio que siempre: hay que decidir antes de las
  -- validaciones de pago, que leen `v_delivery_fee`. `v_delivery_fee > 0` hace
  -- que si la promo de lanzamiento (arriba) ya dejó el envío en 0, este bloque
  ...
  then
    ...
    if v_item_promo_distinct = 1 then
      select mi.free_delivery_days, mi.free_delivery_until
        into v_item_promo_days, v_item_free_delivery_until
        from public.menu_items mi
       where mi.id = (p_items -> 0 ->> 'menu_item_id')::uuid
         and mi.business_id = p_business_id;

      -- Las dos condiciones se combinan por "o": basta con que una aplique.
      if public.menu_item_free_delivery_day(v_item_promo_days)
         or public.menu_item_free_delivery_until(v_item_free_delivery_until)
      then
        v_delivery_fee := 0;
        v_fee_source   := 'promo';
      end if;
...
-- -----------------------------------------------------------------------------
-- `cart_item_free_delivery` (0228), extendida con el mismo "o" que
-- `create_customer_order`. SOLO PARA PINTAR — quien decide el precio sigue
-- siendo `create_customer_order`. El hook de checkout que ya la llama
-- (`use-checkout-state.ts`) no necesita saber que existe esta migración.
-- -----------------------------------------------------------------------------
...
as $function$
  select coalesce(
    ...
    and (
      select
        public.menu_item_free_delivery_day(mi.free_delivery_days)
        or public.menu_item_free_delivery_until(mi.free_delivery_until)
      from public.menu_items mi
...
comment on function public.cart_item_free_delivery(uuid, uuid[]) is
  'Solo para pintar el checkout: ¿esta bolsa (un plato distinto, con envío gratis hoy por calendario semanal o por fecha de corte) se lleva el envío gratis? Quien decide el precio es create_customer_order.';

-- -----------------------------------------------------------------------------
-- El flag real: los dos combos de Al Punto llevan envío GRATIS (no un monto
-- fijo) hasta el 27 de septiembre de 2026 inclusive. La comisión que Tindivo
-- le cobra al negocio (`app_settings.commissions.delivery`, S/1.50, sin
-- override para este negocio) NO SE TOCA: se sigue calculando en
-- `advance_order` al marcar `pickup`, ajena por completo a esta migración.
-- -----------------------------------------------------------------------------
update public.menu_items
   set free_delivery_until = '2026-09-27'
