-- 0148 · El cliente sin GPS que paga por adelantado SI puede pedir.
--
-- QUE ROMPIA. `create_customer_order` se contradecia consigo misma. Su guard de
-- GPS exigia coordenadas a todo metodo distinto de 'failed'; doce lineas mas
-- abajo, la misma funcion trataba 'manual_skip_prepaid' como valor legitimo
-- (marca `gpsFallbackPrepaid`). El canal B2C manda exactamente ese metodo SIN
-- coordenadas en dos sitios de `use-checkout-actions.ts` (67 y 96), y el mas
-- grave es el boton de `GeoBlockView`: la pantalla que sale cuando el GPS
-- fallo, se denego o quedo fuera de zona ofrece "pagar por adelantado" como
-- salida, y esa salida devolvia 422 "Coordenadas GPS del cliente incompletas".
--
-- A quien le pasaba: al vecino con telefono barato o mala senal, que es el
-- perfil del piloto de San Jacinto. Medido el 2026-08-12 recorriendo el flujo
-- real del cliente: el pedido no se creaba hasta conceder el GPS al navegador.
--
-- QUE CAMBIA. UNA condicion. El resto de la funcion es copia literal de la
-- 0143, que era su ultima definicion viva. Idempotente (CREATE OR REPLACE).
--
-- Verificar antes de aplicar en produccion (§2.9 AGENTS.md):
--   SELECT oid, pg_get_function_arguments(oid)
--   FROM pg_proc WHERE proname = 'create_customer_order';
-- Debe devolver UNA sola fila. Si devuelve mas, hay una sobrecarga activa.
