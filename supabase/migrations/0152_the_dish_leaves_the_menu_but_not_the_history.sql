-- =============================================================================
-- 0152 · Soft delete de platos
-- Idempotente.
--
-- Borrar un plato de verdad no se podia: la FK
-- `customer_order_items.menu_item_id` es NO ACTION, asi que en cuanto un plato
-- se vende una vez el DELETE revienta por violacion de clave foranea. Y el
-- dashboard no miraba el error: navegaba a /menu igual y el negocio se
-- quedaba creyendo que lo habia borrado. Hoy no se nota porque todavia no hay
-- ni una linea de pedido en prod; se notaria con el primer pedido real.
--
-- La alternativa era ON DELETE SET NULL, pero eso rompe el enlace con el
-- historial: `item_name_snapshot` conserva el TEXTO de lo vendido, no permite
-- agrupar. Con SET NULL se pierde para siempre poder responder "cuanto he
-- vendido de este plato".
--
-- `deleted_at` NO es lo mismo que `is_available`. `is_available` es "hoy no
-- hay ceviche"; `deleted_at` es "ya no vendo ceviche". Si se mezclan, el plato
-- retirado sigue apareciendole al negocio como si fuera a volver manana.
--
-- Las dos funciones se recrean desde su definicion viva en la base, no desde
-- una migracion anterior, para no revertir sin querer los parches de 0057,
-- 0063 y 0105.
-- =============================================================================
...
comment on column public.menu_items.deleted_at is
  'Retirado de la carta. La fila sobrevive para que el historial de pedidos conserve su enlace.';

-- El menu se lee siempre filtrando por esto, y siempre acotado a un negocio.
create index if not exists menu_items_not_deleted_idx
  on public.menu_items (business_id, category_id)
  where deleted_at is null;

-- ----------------------------------------------------------------------------
-- search_catalog: un plato retirado no puede seguir saliendo en la busqueda.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- create_customer_order: y tampoco se puede pedir. Sin este filtro, un cliente
-- con el catalogo ya cargado en el movil podria pedir algo retirado hace un
-- rato; ahora cae en el mismo error que un plato de otro negocio.
-- ----------------------------------------------------------------------------
