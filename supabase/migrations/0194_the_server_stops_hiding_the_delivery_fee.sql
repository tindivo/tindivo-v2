-- 0194 · El servidor deja de omitir el envio al negar la contraentrega
--
-- QUE CAMBIA
--   Una sola frase dentro de `create_customer_order`. Ni una regla, ni un
--   parametro, ni una columna. El resto del cuerpo es literalmente el que ya
--   corria: esta migracion se genero volcando `pg_get_functiondef` de la
--   definicion viva y tocando esa linea, para no arrastrar por el camino nada
--   de lo que dejaron la 0171, la 0185 y la 0187.
--
-- POR QUE
--   El guard compara `v_order_amount + v_delivery_fee > v_threshold`, o sea que
--   el DELIVERY cuenta para el umbral. El mensaje decia «Pedidos mayores a S/80
--   requieren pago adelantado» y se callaba esa mitad: una bolsa de S/79 con
--   S/2 de envio queda obligada a prepagar mientras el texto habla de S/80. El
--   cliente mira su bolsa, ve 79, y el sistema le parece roto.
--
--   El cliente del checkout ya se corrigio («Tu total con envio pasa de S/80»);
--   esto alinea la ultima linea de defensa, que es la que habla cuando alguien
--   llega por API o cuando el navegador va desincronizado.
--
--   El mensaje ahora lleva LOS DOS numeros —el total real y el umbral— porque
--   quien lo lee esta viendo un rechazo que no esperaba, y saber contra que se
--   comparo es la mitad de la respuesta.
--
-- SIN TILDES EN EL TEXTO DE LA EXCEPCION: viaja por `raise exception` hasta
-- logs y clientes con codificaciones que no controlamos.
--
-- IDEMPOTENTE: `CREATE OR REPLACE`. Los grants no se tocan (un replace los
-- conserva: service_role sigue siendo el unico con EXECUTE).
