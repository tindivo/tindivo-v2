-- =============================================================================
-- 0153 · Fijar search_path en las tres funciones que lo tenian mutable
-- Idempotente.
--
-- El invariante 3 de CLAUDE.md pide `SET search_path = ''` en los helpers
-- SECURITY DEFINER, y estas tres se habian quedado fuera. `get_advisors` las
-- venia marcando como `function_search_path_mutable`.
--
-- Por que importa: una funcion SECURITY DEFINER corre con los permisos de su
-- dueno. Si su search_path lo decide quien la llama, ese llamante puede crear
-- una tabla o una funcion con el mismo nombre en un esquema suyo, ponerlo
-- delante en el search_path y hacer que el cuerpo ejecute SU codigo con
-- permisos ajenos. Con el search_path fijo a vacio, todo nombre tiene que ir
-- cualificado y no hay nada que secuestrar.
--
-- Se comprobo ANTES de tocar nada que las tres ya cualifican todo lo que usan:
--   · tablas: todas con `public.` (`new_phones` y `nightly_orders` son CTEs)
--   · tipos propios: `public.order_status` en los declare
--   · funciones propias: `public.geo_distance_km`, `public.customer_is_blocked`,
--     `public.point_in_coverage_polygon`
--   · el resto son built-ins de pg_catalog, que siguen accesibles con el
--     search_path vacio
-- Por eso anadir la clausula no cambia el comportamiento de ninguna.
--
-- La unica excepcion es `order_cash_owed`, cuyo argumento estaba declarado
-- como `o orders`: sin search_path ese nombre ya no resuelve, asi que pasa a
-- `o public.orders`. Es el mismo tipo, la firma no cambia.
--
-- Los cuerpos salen de su definicion viva en la base, no de una migracion
-- anterior, para no revertir parches acumulados (0057, 0063, 0105, 0152).
-- =============================================================================
...
-- ----------------------------------------------------------------------------
-- order_cash_owed (no es SECURITY DEFINER, pero el lint la marca igual)
-- ----------------------------------------------------------------------------
