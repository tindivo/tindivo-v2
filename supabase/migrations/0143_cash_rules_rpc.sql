-- =============================================================================
-- 0143 · Reglas de efectivo B2C: max_cash_bill, max_change, create_customer_order
-- =============================================================================
--
-- POR QUÉ.
--   El sistema de efectivo B2C tiene dos riesgos sin gestionar:
--   R2: el cliente declara un billete de S/1.000 y el motorizado no trae
--       cambio. Riesgo operativo real: los mozos no llevan más de S/100.
--   R3: el vuelto requerido supera lo que puede acarrear el motorizado
--       en un solo turno. Umbral calibrado con datos legacy: S/50.
--
--   Ambos umbrales son operativos, no de diseño. Se guardan en app_settings
--   para poder ajustarlos sin nueva migración.
--
-- QUÉ CAMBIA.
--   A) Seed de dos keys en app_settings: max_cash_bill = 100, max_change = 50.
--   B) RLS as_public_read: las dos nuevas keys se añaden al whitelist.
--   C) create_customer_order: tres cambios exactos del spec (§B):
--      B.1  Guard explícito pending_mixed con excepción clara.
--      B.2  change_to_give usa round() simple, no greatest(0, round()).
--           (greatest ocultaba vueltos negativos que ya debería rechazar R1.)
--      B.3  Validaciones R2 y R3 con fallback 100/50 si las keys no existen.
--
-- NO CAMBIA.
--   - La firma de la función (no hay DROP, solo CREATE OR REPLACE).
--   - Los grants (service_role mantiene execute, anon/authenticated siguen sin él).
--   - El resto de la lógica de create_customer_order.
--
-- ROLLBACK: supabase/rollbacks/0143_cash_rules_rpc.rollback.sql
-- =============================================================================

-- ─── A. Seed de configuración de efectivo ────────────────────────────────────
INSERT INTO public.app_settings (key, value)
VALUES
  ('max_cash_bill', '100'::jsonb),
  ('max_change',    '50'::jsonb)
ON CONFLICT (key) DO NOTHING;

-- ─── B. RLS as_public_read — añadir las nuevas keys al whitelist ─────────────
...
-- ─── C. create_customer_order con reglas de efectivo B2C ─────────────────────
--
...
-- Debe devolver UNA sola fila. Si devuelve más, hay una sobrecarga activa.
-- (§2.9 AGENTS.md)
...
-- Los grants siguen igual que en 0105: service_role mantiene execute,
-- anon/authenticated siguen sin acceso directo (pasan por la API).
