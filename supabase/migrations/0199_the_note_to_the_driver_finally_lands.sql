-- =============================================================================
-- 0199 · La nota al motorizado, por fin con un camino de escritura
-- =============================================================================
--
-- Recoge la deuda que la 0127 dejó anotada al matar `p_notes`. Aquel decía,
-- literalmente:
--
--   «Una nota dirigida al MOTORIZADO —"tocar el timbre de la casa azul",
--    "llamar al llegar", "hay perro"— es una idea razonable y NO forma parte de
--    este cambio. Si se retoma, hay que diseñarla de cero: quién la escribe,
--    quién la ve y en qué pantalla. Lo que NO debe hacerse es resucitar
--    `p_notes` como estaba, porque su problema no era el nombre sino que no
--    llevaba a ninguna parte.»
--
-- Las tres preguntas, contestadas:
--
--   QUIÉN LA ESCRIBE · el cliente, en el checkout, en la tarjeta de entrega,
--     junto a la dirección a la que se refiere. Es opcional y no bloquea nada.
--   QUIÉN LA VE · el motorizado. `apps/motorizados` YA la pinta —«Nota del
--     cliente: …» en `components/order/order-detail.tsx`— leyendo
--     `customerNotes`, que el endpoint `/driver/orders/[id]` YA devuelve desde
--     `orders.customer_notes`. Lo único que faltaba era que alguien escribiera
--     en esa columna.
--   EN QUÉ PANTALLA · en la del pedido asignado, la que el motorizado tiene
--     abierta cuando sale a repartir.
--
-- O sea: al revés que `p_notes`, esto no es un parámetro buscando destino. El
-- destino existía y estaba pintado; faltaba el camino de escritura.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- POR QUÉ HAY `DROP FUNCTION` Y NO SOLO `CREATE OR REPLACE`
--
--   La firma cambia (18 -> 19 argumentos). En Postgres eso NO reemplaza: crea
--   una función nueva y deja viva la vieja. Y aquí es peor que en la 0127,
--   porque el parámetro nuevo lleva DEFAULT: con las dos presentes, la llamada
--   por nombre de 18 argumentos que hace el API resolvería a las dos y
--   Postgres la rechazaría por ambigua («function is not unique»). Se dejaría
--   de poder pedir.
--
--   Mismo motivo que en la 0126 y la 0127, y el mismo precedente:
--   `register_appeal_refund` en 0073/0077.
--
--   ACL real leída ANTES de esta migración:
--     {postgres=X/postgres, service_role=X/postgres}
--   service_role=true · anon=false · authenticated=false. Sigue siendo
--   `SECURITY DEFINER` con `SET search_path = ''`.
--
--   OJO CON EL REVOKE — `FROM PUBLIC` NO BASTA, Y ESTO SE MIDIÓ.
--     Supabase tiene default privileges para funciones creadas por `postgres`
--     en este esquema:
--       postgres|f|{postgres=X/postgres, anon=X/postgres,
--                   authenticated=X/postgres, service_role=X/postgres}
--     así que la función nace con EXECUTE **concedido explícitamente** a `anon`
--     y `authenticated`. `REVOKE ... FROM PUBLIC` quita el grant de PUBLIC y
--     deja los otros dos intactos: se comprobó aplicando esta migración con
--     solo ese REVOKE y leyendo el ACL, que salió
--       {postgres=X, anon=X, authenticated=X, service_role=X}
--
--     Y eso aquí no es cosmético. La función es SECURITY DEFINER y recibe
--     `p_customer_user_id` como PARÁMETRO: con EXECUTE para `anon`, cualquiera
--     con la clave publicable podría crear pedidos a nombre de otro usuario
--     saltándose el endpoint entero —y con él el guard de teléfono verificado,
--     el de contraentrega y el de horario, que viven en el API, no en la RPC.
--
--     Por eso el REVOKE nombra los tres, que es además la forma que ya usa el
--     resto del repo (0059, 0066, 0067…).
--
-- ─────────────────────────────────────────────────────────────────────────────
-- MÉTODO DEL CUERPO
--
--   Generado desde el texto EXACTO de la definición viva
--   (md5 136cdb4ceb8fa38fba109fb84ccf432e, 603 líneas) con TRES sustituciones
--   y cuatro aserciones automáticas: que `customer_notes` no aparecía en
--   ninguna parte antes, que cada ancla existía exactamente una vez, que el
--   resultado tiene exactamente dos líneas más, y que **el resto del cuerpo es
--   idéntico línea por línea**.
--
--   Esta migración NO toca el cálculo del envío, ni la banda, ni la promo, ni
--   los guards de contraentrega, ni los payloads de eventos. Solo la firma y
--   el INSERT de `public.orders`.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- EL SANEO, Y POR QUÉ VA EN LA BASE
--
--     left(nullif(btrim(regexp_replace(coalesce(p_customer_notes,''),
--                                      '\s+', ' ', 'g')), ''), 200)
--
--   · `coalesce` + `nullif(btrim(...))` · una nota que llega vacía o solo con
--     espacios se guarda como NULL, no como ''. Así la condición del motorizado
--     —`{order.customerNotes && ...}`— sigue siendo la misma pregunta, y no
--     aparece una etiqueta «Nota del cliente:» seguida de nada.
--   · `regexp_replace('\s+', ' ')` · el campo es un textarea y llegan saltos de
--     línea. La ficha del motorizado los pinta en una línea de texto corrida.
--   · `left(..., 200)` · un tope. La columna es `text` sin límite y esto lo
--     escribe el público; 200 caracteres es de sobra para «tocar el timbre dos
--     veces, el perro ladra pero no muerde» y cierra la puerta a que alguien
--     use el campo como almacén.
--
--   Va aquí y no solo en el navegador porque el navegador se puede saltar: el
--   contrato Zod del API pone el mismo tope, pero la última palabra sobre lo
--   que entra en la columna la tiene la función.
--
-- =============================================================================


-- ── 1 · Fuera la firma de 18 argumentos --------------------------------------
...
-- ── 2 · La firma nueva, de 19 argumentos, con `p_customer_notes` -------------
CREATE OR REPLACE FUNCTION public.create_customer_order(p_business_id uuid, p_customer_user_id uuid, p_delivery_method delivery_method, p_payment_intent payment_intent, p_customer_name text, p_customer_phone text, p_items jsonb, p_delivery_address text, p_delivery_reference text, p_delivery_lat numeric DEFAULT NULL::numeric, p_delivery_lng numeric DEFAULT NULL::numeric, p_source order_source DEFAULT 'customer_pwa'::order_source, p_client_pays_with numeric DEFAULT NULL::numeric, p_customer_gps_lat double precision DEFAULT NULL::double precision, p_customer_gps_lng double precision DEFAULT NULL::double precision, p_customer_gps_accuracy_m double precision DEFAULT NULL::double precision, p_customer_gps_distance_to_center_km numeric DEFAULT NULL::numeric, p_customer_gps_method text DEFAULT NULL::text, p_customer_notes text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
GRANT EXECUTE ON FUNCTION public.create_customer_order(
  uuid, uuid, public.delivery_method, public.payment_intent, text, text, jsonb,
  text, text, numeric, numeric, public.order_source, numeric,
  double precision, double precision, double precision, numeric, text, text) TO service_role;
