-- =============================================================================
-- 0021 · Validación por llamada (estado `validando`) — Documento Maestro §4
-- Un pedido de CONTRAENTREGA entra a `validando` si: número nuevo (primer pedido
-- del teléfono) · número/dirección con strike · monto grande. La cajera valida
-- por llamada (validate_order pass) → pending_acceptance, o rechaza/expira (fail)
-- → cancelled. Prepago se mantiene en pending_acceptance hasta cablear comprobante.
-- Inngest: orderValidationTimeout (5 min) cancela si nadie valida.
-- =============================================================================

insert into public.app_settings (key, value) values
  ('validation', '{"amountThreshold": 80}'::jsonb)
on conflict (key) do nothing;

-- create_customer_order: decide el estado inicial (validando vs pending_acceptance).
...
begin
  ...
  -- Estado inicial: contraentrega de número nuevo / con strike / monto grande -> validando.
...
-- La cajera/negocio valida (pass) o rechaza (fail) un pedido en `validando`.
...
begin
  ...
  if p_pass then
    update public.orders set status = 'pending_acceptance' where id = p_order_id;
    insert into public.domain_events (aggregate_type, aggregate_id, event_type, payload)
    ...
    update public.orders
      set status = 'cancelled', cancel_reason = 'validation_timeout',
          cancelled_by = p_actor_user_id, cancel_note = p_reason
      where id = p_order_id;
    insert into public.domain_events (aggregate_type, aggregate_id, event_type, payload)
...
revoke execute on function public.validate_order(uuid, uuid, public.user_role, boolean, text) from public, anon, authenticated;
grant execute on function public.validate_order(uuid, uuid, public.user_role, boolean, text) to service_role;
