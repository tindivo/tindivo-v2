-- =============================================================================
-- 0207 · El motorizado sabe de qué calidad es el punto al que va
-- =============================================================================
--
-- QUÉ FALTABA. La 0202 le dio a `customer_addresses` dos columnas para poder
-- distinguir un GPS de ±8 m de un pin puesto a mano y de un punto que no eligió
-- nadie. Esa distinción existe desde entonces… y muere en la libreta del
-- cliente: el pedido lleva `delivery_coordinates_lat/lng` y nada más, así que
-- quien conduce ve un punto y no tiene forma de saber si fiarse de él o leer la
-- referencia. Con CINCO direcciones apuntando ahora mismo al centro de
-- San Jacinto, eso no es teórico.
--
-- POR QUÉ VIAJA COMO PARÁMETRO Y NO SE MIRA LA DIRECCIÓN. `orders` no guarda a
-- qué fila de `customer_addresses` corresponde el punto —solo la coordenada—,
-- así que no hay a quién preguntarle. Y aunque lo guardara, tampoco convendría:
-- el pedido tiene que fotografiar la calidad del punto EN EL MOMENTO en que se
-- pidió. Si el cliente corrige su dirección tres días después, el pedido de
-- anteayer no debe cambiar de historia. Es el mismo criterio con el que ya
-- viajan los cinco `p_customer_gps_*`.
--
-- OJO, NO SON LO MISMO QUE `customer_gps_accuracy_m`. Aquella es la lectura
-- ANTIFRAUDE: dónde estaba parado el cliente al pedir. Esta es la calidad del
-- punto de ENTREGA. Un cliente puede pedir desde el trabajo con un GPS
-- excelente y mandar el pedido a una casa cuyo pin puso a dedo.
--
-- LOS DOS SIGNIFICADOS DE NULL, iguales que en la 0202 y en la 0147:
--
--   delivery_location_confirmed_at NULL -> nadie eligió ese punto. El
--     motorizado tiene que guiarse por la referencia de texto.
--   delivery_coordinates_accuracy_m NULL con confirmed_at NO nulo -> lo puso
--     una persona moviendo el mapa. No hay medida porque no hubo sensor: no es
--     «desconocido», es «esto es una decisión, no una medición».
--
-- METADATO QUE NO PUEDE TUMBAR UNA VENTA. La columna lleva su CHECK (> 0),
-- pero la función NO deja que un valor absurdo llegue hasta él: `nullif(
-- greatest(coalesce(...,0),0),0)` convierte 0, negativo o nulo en NULL. Si el
-- cliente mandara basura, se pierde el dato y el pedido entra igual. Al revés
-- —dejar que el CHECK reviente el INSERT— sería que un adorno impida pedir.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- POR QUÉ HAY `DROP FUNCTION` Y NO SOLO `CREATE OR REPLACE`
--
--   La firma pasa de 19 a 21 argumentos, y en Postgres eso no reemplaza: crea
--   una función nueva y deja viva la vieja. Con las dos presentes y todos los
--   parámetros nuevos con DEFAULT, la llamada por nombre del API resolvería a
--   las dos y Postgres la rechazaría por ambigua. Se dejaría de poder pedir.
--   Mismo motivo y mismo precedente que 0126, 0127 y 0199.
--
--   ACL real leída ANTES de esta migración:
--     {postgres=X/postgres, service_role=X/postgres}
--   `REVOKE ... FROM PUBLIC` NO BASTA: los default privileges de Supabase
--   conceden EXECUTE a `anon` y `authenticated` a toda función nueva del
--   esquema, y esta es SECURITY DEFINER con `p_customer_user_id` como
--   PARÁMETRO. Por eso el REVOKE los nombra a los tres, como en 0199.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- MÉTODO DEL CUERPO
--
--   Generado desde el texto de la 0199 —que reproduce la definición viva byte a
--   byte: md5(prosrc) = 92d822551ce7a26ce8b236350d07fa60 en local, en prod y en
--   el fichero— con TRES sustituciones y tres aserciones automáticas: que cada
--   ancla existe una sola vez, que el resultado tiene exactamente tres líneas
--   más, y que **quitando las líneas nuevas se recupera ese mismo md5**, o sea
--   que el resto es idéntico línea por línea.
--
--   Esta migración NO toca el cálculo del envío, ni la banda, ni la promo, ni
--   los guards de contraentrega, ni los payloads de eventos. Solo la firma y el
--   INSERT de `public.orders`.
--
-- ROLLBACK: supabase/rollbacks/0207_the_driver_learns_how_good_the_point_is.rollback.sql
-- =============================================================================


-- ── 1 · Las dos columnas ─────────────────────────────────────────────────────
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS delivery_coordinates_accuracy_m integer,
  ADD COLUMN IF NOT EXISTS delivery_location_confirmed_at timestamptz;

COMMENT ON COLUMN public.orders.delivery_coordinates_accuracy_m IS
  'Metros de precisión del punto de ENTREGA, fotografiados al pedir. NULL = el pin se puso a mano (no hubo sensor). No confundir con customer_gps_accuracy_m, que es la lectura antifraude de dónde estaba el cliente al pedir.';

COMMENT ON COLUMN public.orders.delivery_location_confirmed_at IS
  'Cuándo una persona confirmó el punto de entrega en el mapa. NULL = ese punto no lo eligió nadie (0202); el motorizado debe guiarse por la referencia.';

-- Misma precisión imposible que en la 0202: 0 era el centinela del legacy que
-- destruyó la medida de 49 filas (0147). Aquí la ausencia de medida es NULL.
ALTER TABLE public.orders
  DROP CONSTRAINT IF EXISTS orders_delivery_accuracy_positive;
ALTER TABLE public.orders
  ADD CONSTRAINT orders_delivery_accuracy_positive
  CHECK (delivery_coordinates_accuracy_m IS NULL OR delivery_coordinates_accuracy_m > 0);


-- ── 2 · Fuera la firma de 19 argumentos ──────────────────────────────────────
...
-- ── 3 · La de 21, con la calidad del punto ───────────────────────────────────
