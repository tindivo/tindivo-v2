...
-- =============================================================================
--
-- Cierra la deuda anotada en la cabecera de la 0126 y en el reconocimiento de
-- la Parte D.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- QUÉ ERA `p_notes` — MEDIDO, NO DEDUCIDO
--
--   Un parámetro que existe en la firma desde la 0080 y que EL CUERPO NUNCA
--   REFERENCIÓ. Contado sobre `pg_get_functiondef` en prod, tras la 0126:
--   aparece EXACTAMENTE UNA VEZ en toda la definición, y esa vez es la propia
--   declaración del parámetro.
--
--   El endpoint lo venía enviando en cada llamada. O sea que durante 47
--   migraciones cualquier cosa que se escribiera ahí se descartaba en silencio:
--   ni error, ni columna, ni log. Se aceptaba y se tiraba.
--
--   Hoy en la práctica ni siquiera llega texto: el formulario de `apps/negocios`
--   no tiene campo de notas —barrido por `notes|notas` sobre `features/nuevo/`,
--   cero resultados— y ningún otro cliente hace POST a `/business/orders`. El
--   `notes` del zod era un contrato sin emisor ni receptor.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- LO QUE ESTA MIGRACIÓN NO DECIDE — LA NOTA AL MOTORIZADO
--
--   `p_notes` se elimina por estar MUERTO, no porque la idea de una nota sea
--   mala. Son cosas distintas y conviene no confundirlas al leer este archivo
--   dentro de seis meses.
--
--   Una nota dirigida al MOTORIZADO —"tocar el timbre de la casa azul", "llamar
--   al llegar", "hay perro"— es una idea razonable y NO forma parte de este
--   cambio. Si se retoma, hay que diseñarla de cero: quién la escribe, quién la
--   ve y en qué pantalla. Lo que NO debe hacerse es resucitar `p_notes` como
--   estaba, porque su problema no era el nombre sino que no llevaba a ninguna
--   parte.
--
--   Dato útil para ese día: `public.orders` YA TIENE tres columnas de notas sin
--   usar por este flujo — `customer_notes`, `business_notes` y `driver_notes`,
--   las tres `text` nullable. Una nota al motorizado tendría dónde vivir sin
--   añadir columna; lo que falta es el camino de escritura y la UI.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- POR QUÉ HAY `DROP FUNCTION` Y NO SOLO `CREATE OR REPLACE`
--
--   La firma cambia (13 -> 12 argumentos). En Postgres eso no reemplaza: crea
--   una función nueva y deja viva la vieja. Con las dos presentes, la llamada de
--   13 argumentos seguiría resolviendo a la de siempre y esta migración no
--   habría servido de nada. Mismo motivo que en la 0126, y el mismo precedente:
--   `register_appeal_refund` en 0073/0077.
--
--   ACL real leída de prod ANTES de esta migración:
--     {postgres=X/postgres, service_role=X/postgres}
--   service_role=true · anon=false · authenticated=false. La firma nueva nacería
--   con EXECUTE para PUBLIC si no se emitieran los grants de abajo.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- MÉTODO DEL CUERPO
--
--   Generado desde el texto EXACTO de la 0126 (líneas 116-273, la definición
--   viva en prod, md5 d0cd72f0123aa88c8ee8abcb3d3b4595) con UNA sustitución en
--   la línea de la firma. Tres aserciones: que `p_notes` aparecía exactamente
--   una vez antes, que no sobrevive ninguna después, y que **el resto del
--   cuerpo es idéntico línea por línea** — esta migración no toca ni el cálculo
--   del envío, ni la banda, ni los payloads de eventos.
--
-- =============================================================================


-- ── 1 · Fuera la firma de 13 argumentos --------------------------------------
...
-- ── 2 · La firma nueva, de 12 argumentos, sin `p_notes` ----------------------
...
-- ── 3 · Grants ---------------------------------------------------------------
-- La firma cambió, así que la ACL NO se hereda. Sin estas dos líneas la función
-- nueva queda ejecutable por PUBLIC. REGLA de Docs/RIESGOS-LEDGER.md: toda
-- migración que cambie una firma emite REVOKE + GRANT en el mismo archivo, sin
-- excepción. Aplica igual aunque este cambio no mueva dinero: sigue siendo
-- SECURITY DEFINER en la cadena de creación de pedidos.
...
-- Si el DROP del paso 1 fallara en silencio, quedarían dos candidatas y la
-- llamada de 13 argumentos seguiría resolviendo a la vieja. Esto lo convierte en
-- un fallo ruidoso al aplicar, que es cuando sale barato arreglarlo.
DO $$
DECLARE
  v_n int;
BEGIN
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION
      '0127 abortada: quedan % sobrecargas de create_business_manual_order, se esperaba exactamente 1',
