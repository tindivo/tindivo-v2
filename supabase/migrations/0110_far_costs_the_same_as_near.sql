-- =============================================================================
-- 0110 · Una entrega lejana cuesta al negocio lo mismo que una cercana
-- =============================================================================
--
-- EL PROBLEMA, MEDIDO
-- El recorrido de la Parte 4 dio este número:
--
--   banda | cliente pagó | cobrado al negocio | diferencia
--   near  |         2.00 |               2.00 |       0.00
--   far   |         2.00 |               2.50 |     +0.50
--
-- La creación del pedido nunca calcula banda: `create_customer_order` y
-- `create_business_manual_order` toman siempre `delivery_bands.near`. El
-- cliente paga 2.00 vaya donde vaya. Pero al recoger, el motorizado declara la
-- banda y de ahí salía el envío cobrado al negocio. Resultado: el negocio
-- absorbía S/0.50 por cada entrega lejana, sin haber decidido nada.
--
-- DOS CAMBIOS
--
-- 1. `advance_order`, acción 'pickup': el envío cobrado sale del pedido
--    (`orders.delivery_fee`, lo que el cliente pagó), no de la banda. La banda
--    se sigue guardando en `delivery_distance_band` — es el dato que hará
--    falta para diseñar 'far' de verdad.
--
-- 2. `commissions.far` baja de 4.00 a 3.50, igualando a `near`.
--
--    ESTO ES TEMPORAL. Mientras el lado del cliente no distinga banda y siga
--    cobrando 2.00 siempre, cobrar más al negocio por una entrega lejana es
--    cobrarle por una decisión que no tomó. Cuando el cliente pague 2.50 por
--    las lejanas, `commissions.far` vuelve a 4.00 y el recargo se sostiene
--    solo. Hasta entonces: mismo precio.
--
-- POR QUÉ VA EN UNA MIGRACIÓN Y NO COMO DATO
-- Las tarifas se habían ajustado con un UPDATE suelto. El `supabase db reset`
-- de la sesión anterior las revirtió a los valores de las migraciones de seed
-- (0006/0086/0087) sin que nadie se enterara, y las dos bases estuvieron con
-- tarifas distintas. Una auditoría de las 17 claves de `app_settings` confirmó
-- que `commissions` era la única que divergía sin repararse sola:
-- `platform_schedule` y `order_intake_cutoff` también divergen, pero el seed
-- de E2E las vuelve a fijar a propósito (24/7 en local).
--
-- MÉTODO: cuerpo reproducido desde la definición viva (pg_get_functiondef) con
-- sustituciones verificadas por aserción sobre el texto exacto de cada línea.

-- 1. Tarifas. `delivery_bands` no cambia; se deja explícito para que el valor
--    quede fijado por migración y no dependa de un UPDATE manual.
UPDATE public.app_settings
   SET value = '{"pickup": 1.00, "near": 3.50, "far": 3.50}'::jsonb
 WHERE key = 'commissions';

UPDATE public.app_settings
   SET value = '{"near": 2.00, "far": 2.50}'::jsonb
 WHERE key = 'delivery_bands';

-- 2. advance_order: el envío cobrado sale del pedido, no de la banda.
