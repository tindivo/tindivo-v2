...
import { type DatePreset, getLimaDate, getPresetRange } from '@/lib/order-history/date-utils'
import { useHistory } from '@/lib/order-history/use-history'
...
export function HistorialList() {
  ...
  const [{ start, end }, setRange] = useState(() => getPresetRange('today'))

  const { rows, loading, error } = useHistory(start, end)
  ...
  const isTodayOnly = start === end && start === todayStr
  ...
  const allDisplayRows = useMemo(
    () => rows.map((r) => toDisplay(r, isTodayOnly)),
    [rows, isTodayOnly],
  )
  ...
  return (
    <>
      ...
      {loading ? (
        <div className="p-10 text-center text-ink-muted">Cargando pedidos…</div>
      ) : (
        <>
          <div className="mb-4">
            <HistorySummary rows={allDisplayRows} isTodayOnly={isTodayOnly} />
          </div>
          ...
          <HistoryList rows={visibleRows} onSelect={handleSelect} isTodayOnly={isTodayOnly} />
        </>
      )}
    </>
  )
