            ...
            {qrs.length > 1 && (
              <div className="flex rounded-lg bg-ink/[0.06] p-0.5">
                {qrs.map((q) => (
                  ...
                  >
                    {walletLabel(q.wallet)} {q.slot === 1 ? '(Principal)' : '(Respaldo)'}
                  </button>
                ))}
              </div>
            )}
          ...
          {currentQr ? (
            <Card className="p-4 bg-surface-low border border-border">
              <div className="flex flex-col items-center text-center">
                {/* Imagen del QR */}
                {currentQr.qrUrl ? (
                  <div className="relative mb-3 flex h-48 w-48 items-center justify-center rounded-2xl bg-white p-2.5 shadow-sm border border-border">
                    <img
                      src={currentQr.qrUrl}
                ...
                <div className="inline-flex items-center gap-1.5 rounded-full bg-brand-light px-2.5 py-0.5 text-xs font-bold text-brand-dark mb-1">
                  ...
                  {walletLabel(currentQr.wallet)} {currentQr.isDefault ? '· Principal' : ''}
                </div>
                ...
                <div className="mt-2.5 flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-2 border border-border shadow-xs w-full max-w-[280px]">
                  ...
                  <button
                    ...
                    className="flex h-7 items-center gap-1 rounded-lg bg-ink/[0.06] px-2 text-[11px] font-semibold text-ink transition-colors hover:bg-ink/[0.12] active:scale-95 cursor-pointer"
                  >
          ...
          {business.address && (
            <div className="flex items-start gap-2.5 rounded-xl bg-surface-low p-3 border border-border">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[13px] font-medium text-ink leading-snug">{business.address}</p>
              </div>
            </div>
          )}
