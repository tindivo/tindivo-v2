...
const Schema = z.object({ confirmedAmount: z.number().nonnegative() })
...
export async function POST(
  ...
  try {
    ...
    const { data, error } = await service.rpc('confirm_cash_settlement', {
      p_id: id,
      p_business_user_id: user.id,
      p_confirmed_amount: body.confirmedAmount,
    })
    if (error) {
