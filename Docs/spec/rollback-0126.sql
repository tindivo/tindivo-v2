-- =============================================================================
-- ROLLBACK de la migración 0126
-- =============================================================================
--
-- 0126 · La cajera elige la banda, y el envío del pedido manual la respeta.
--
-- Deshace las DOS cosas que hizo 0126: la firma de 13 argumentos (vuelve a la
-- de 12) y el cálculo del envío (vuelve a leer siempre `delivery_bands.near`).
--
-- ─────────────────────────────────────────────────────────────────────────────
-- PASO 1 — PRIMERO EL CÓDIGO, DESPUÉS LA BASE
--
--   El Zod del endpoint exige `deliveryDistanceBand` y la llamada al RPC pasa
--   `p_delivery_distance_band`. Si se revierte el esquema sin revertir el
--   código, TODA creación de pedido manual falla: PostgREST no encontraría una
--   función con ese parámetro. No es degradación silenciosa — es caída total
--   del alta de pedidos por teléfono, que es el flujo principal del piloto.
--
--       git revert <sha-del-commit-de-0126>
--
--   Ese revert cubre:
--       supabase/migrations/0126_the_cashier_picks_the_band.sql
--       apps/api/app/api/v1/business/orders/route.ts
--       apps/negocios/features/nuevo/hooks/use-create-order.ts
--       packages/supabase/src/database.types.ts
--
-- PASO 2 — el SQL de abajo, en una sola transacción.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- ⚠️ LO QUE ESTE ROLLBACK NO DESHACE
--
--   Los pedidos creados bajo 0126 conservan su `delivery_distance_band` y su
--   `delivery_fee_source`, y el envío que ya se les cobró queda como está. Son
--   datos ya asentados: reescribirlos sería peor que la inconsistencia. Tras el
--   rollback esas dos columnas simplemente dejan de escribirse.
--
--   Consecuencia concreta: un pedido creado como 'far' con envío S/ 2.50 y aún
--   no entregado conserva ese 2.50 en `orders.delivery_fee`, y
--   `generate_delivery_charges` lo cobrará tal cual al entregarse. Es lo
--   correcto: es lo que se le dijo al cliente.
--
-- =============================================================================
...
-- ── 2 · La firma de 12 argumentos, tal como vivía antes de 0126 --------------
-- Copia literal del bloque de
-- supabase/migrations/0117_queue_lead_and_customer_eta.sql (líneas 431-548),
-- que era la definición viva en prod, md5 c59429138c3f557dc0ec3ea74c3eb444.
...
-- ── 3 · Grants originales ----------------------------------------------------
-- La ACL exacta que tenía la firma de 12 argumentos antes de 0126:
--   {postgres=X/postgres, service_role=X/postgres, supabase_auth_admin=X/postgres}
-- Se restaura entera, `supabase_auth_admin` incluido, para que el estado quede
-- idéntico al de antes y no "casi idéntico".
...
--     -> UNA fila, 12 argumentos, service_role=true, anon=false
--
--   SELECT pg_get_functiondef(oid) LIKE '%(v_bands ->> ''near'')::numeric%'
--     FROM pg_proc WHERE proname='create_business_manual_order' AND prokind='f';
--     -> true  (el envío vuelve a estar clavado a `near`)
