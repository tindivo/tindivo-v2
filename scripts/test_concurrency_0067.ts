...
async function runConcurrencyTest() {
  ...
  if (orderErr || !orders || orders.length === 0) {
    console.error(
      'No se encontró pedido fixture en proof_rejected_final para la prueba de concurrencia.',
    )
    return
  }
  ...
  console.log(
    `Ejecutando create_appeal_report simultáneamente en 2 sesiones autenticadas del cliente...`,
  )

  // Peticiones paralelas simultáneas con Promise.all
  const [res1, res2] = await Promise.all([
    client1.rpc('create_appeal_report', {
      p_order_id: orderId,
      p_description: 'Apelación Cliente Sesión Concurrente 1',
    }),
    client2.rpc('create_appeal_report', {
      p_order_id: orderId,
      p_description: 'Apelación Cliente Sesión Concurrente 2',
    }),
  ])
  ...
  console.log(
    `Conteo final de reportes 'rejected_proof_disputed' para el pedido ${orderId}: ${count}`,
  )

  if (count === 1) {
    console.log(
      '✅ PRUEBA DE CONCURRENCIA EXITOSA: Se garantizó exactamente un solo reporte por pedido.',
    )
  } else {
    console.error(
      `❌ PRUEBA DE CONCURRENCIA FALLIDA: Se encontraron ${count} reportes (esperado 1).`,
    )
  }
}
