...
BEGIN
  ...
  -- Pedido cancelado reciente (dentro de las 24h)
  INSERT INTO public.orders (
    id, short_id, business_id, customer_user_id, status, payment_intent,
    cancel_reason, cancelled_at, order_amount, delivery_fee
  ) VALUES (
    v_order_id_recent, 'REC01', v_business_id, v_customer_id, 'cancelled', 'prepaid',
    'proof_rejected_final', now() - interval '2 hours', 40.00, 5.00
  );

  -- Pedido cancelado hace más de 24h sin apelación
  INSERT INTO public.orders (
    id, short_id, business_id, customer_user_id, status, payment_intent,
    cancel_reason, cancelled_at, order_amount, delivery_fee
  ) VALUES (
    v_order_id_due, 'DUE01', v_business_id, v_customer_id, 'cancelled', 'prepaid',
    'proof_rejected_final', now() - interval '26 hours', 50.00, 5.00
  );

  -- Pedido cancelado hace más de 24h CON apelación creada
  INSERT INTO public.orders (
    id, short_id, business_id, customer_user_id, status, payment_intent,
    cancel_reason, cancelled_at, order_amount, delivery_fee
  ) VALUES (
    v_order_id_appealed, 'APP01', v_business_id, v_customer_id, 'cancelled', 'prepaid',
    'proof_rejected_final', now() - interval '26 hours', 60.00, 5.00
  );

  -- 3. PRUEBA DE OUTBOX TRANSACTIONAL VÍA TRIGGER DE CANCELACIÓN
  SELECT COUNT(*) INTO v_outbox_count
  FROM public.outbox_events
  WHERE event_type = 'order/proof-rejected-final'
    AND payload->>'orderId' IN (v_order_id_recent::text, v_order_id_due::text, v_order_id_appealed::text);

  ASSERT v_outbox_count = 3, 'OUTBOX TRIGGER FALLÓ: Deben haberse creado 3 eventos outbox atómicamente al cancelar los pedidos';
  RAISE NOTICE '[OK] Transactional Outbox Trigger verificado: 3 eventos encolados en la misma transacción.';
  ...
  -- 5. PRUEBA DE FALLBACK CON APELACIÓN PREVIA DEL CLIENTE (DENEGADO)
  -- Crear apelación voluntaria
  INSERT INTO public.reports (
    type, status, order_id, business_id, customer_user_id, created_by, appeal_status
  ) VALUES (
    'rejected_proof_disputed', 'open', v_order_id_appealed, v_business_id, v_customer_id, v_customer_id, 'pending'
  );
  ...
  -- Comprobar que solo existe 1 reporte de fallback
  SELECT COUNT(*) INTO v_report_count
  FROM public.reports
  WHERE order_id = v_order_id_due AND type = 'prepay_refund_review';

  ASSERT v_report_count = 1, 'UNICIDAD FALLÓ: Debe existir exactamente 1 reporte prepay_refund_review';
